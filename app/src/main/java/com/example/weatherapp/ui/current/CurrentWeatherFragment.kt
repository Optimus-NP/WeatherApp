...
class CurrentWeatherFragment : Fragment() {
    ...
    private var temperatureUnit: TemperatureUnit = TemperatureUnit.fromSymbol(Settings.TEMPERATURE_UNIT.defaultValue)!!
    ...
    override fun onCreateView(
        ...
        temperatureUnit = getTemperatureSetting(requireActivity().contentResolver)
    ...
    private fun fetchWeatherData(recyclerView: RecyclerView) {
        ...
        cursor.setNotificationUri(requireActivity().contentResolver, WeatherDataContentProvider.WEATHER_CONTENT_URI)
    ...
    class WeatherAdapter(private val cursor: Cursor, private val temperatureUnit: TemperatureUnit) : RecyclerView.Adapter<WeatherAdapter.WeatherViewHolder>() {
        ...
        inner class WeatherViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            val temperatureFeelLikeTextView: TextView = itemView.findViewById(R.id.tvTemperatureFeelLike)
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WeatherViewHolder {
            val view = LayoutInflater.from(parent.context).inflate(R.layout.city_weather_card, parent, false)
        ...
        override fun onBindViewHolder(holder: WeatherViewHolder, position: Int) {
            if (cursor.moveToPosition(position)) {
                val city = cursor.getString(cursor.getColumnIndexOrThrow(WeatherDatabaseHelper.WeatherDataTable.COLUMN_CITY))
                val temperature = TemperatureUnit.KELVIN.convert(cursor.getDouble(cursor.getColumnIndexOrThrow(WeatherDatabaseHelper.WeatherDataTable.COLUMN_TEMPERATURE)), temperatureUnit)
                val temperatureFeelLike = TemperatureUnit.KELVIN.convert(cursor.getDouble(cursor.getColumnIndexOrThrow(WeatherDatabaseHelper.WeatherDataTable.COLUMN_TEMPERATURE_FEEL_LIKE)), temperatureUnit)
                val weatherCondition = cursor.getString(cursor.getColumnIndexOrThrow(WeatherDatabaseHelper.WeatherDataTable.COLUMN_WEATHER_CONDITION))
                val timestamp = cursor.getLong(cursor.getColumnIndexOrThrow(WeatherDatabaseHelper.WeatherDataTable.COLUMN_TIMESTAMP))

                holder.cityImageView.setImageDrawable(
                    city_drawables.get(City.fromCityName(city))?.let {
                        ResourcesCompat.getDrawable(holder.cityImageView.resources,
                            it, null)
                    }
                )

                val citySpannableString = SpannableString("City: $city")
                citySpannableString.setSpan(StyleSpan(Typeface.BOLD), 0, 4, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                holder.cityTextView.text = citySpannableString

                val temperatureSpannableString = SpannableString("Temperature: ${"%.2f".format(temperature)} ${temperatureUnit.symbol}")
                temperatureSpannableString.setSpan(StyleSpan(Typeface.BOLD), 0, 12, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE) // "Temperature" bold
                holder.temperatureTextView.text = temperatureSpannableString

                val temperatureFeelLikeSpannableString = SpannableString("Temperature Feel Like: ${"%.2f".format(temperatureFeelLike)} ${temperatureUnit.symbol}")
                temperatureFeelLikeSpannableString.setSpan(StyleSpan(Typeface.BOLD), 0, 21, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE) // "Temperature Feel Like" bold
                holder.temperatureFeelLikeTextView.text = temperatureFeelLikeSpannableString

                val weatherConditionSpannableString = SpannableString("Weather Condition: $weatherCondition")
                weatherConditionSpannableString.setSpan(StyleSpan(Typeface.BOLD), 0, 17, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE) // "Weather Condition" bold
                holder.weatherConditionTextView.text = weatherConditionSpannableString

                val timeSpannableString = SpannableString("Time: ${convertTimestampToDate(timestamp)}")
                timeSpannableString.setSpan(StyleSpan(Typeface.BOLD), 0, 5, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE) // "Time:" bold
